...
`./mvnw clean verify -q` also generates JaCoCo HTML, XML, and CSV reports under `target/site/jacoco/`.
...
The application reads database settings from `SPRING_DATASOURCE_URL` (optional, defaults only to the local host/port/database), `SPRING_DATASOURCE_USERNAME`, and `SPRING_DATASOURCE_PASSWORD` (required). JWT discovery uses `JWT_JWK_SET_URI`. The runtime fails at startup when required database credentials are missing. Docker Compose variables are development-only and are not production configuration. Export size is configurable with `AUDIT_EXPORT_MAX_RECORDS` and defaults to 10,000 records; oversized exports return `413 Payload Too Large`. Do not commit credentials or tokens.
